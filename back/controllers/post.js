...
exports.createPost = (req, res, next) => {
  ...
  const post = new Post({
    //... premet d'attribuer toutes propriétés d'un objet (postObject) à un autre objet (sauce)
    ...postObject,
  ...
  post
    ...
    .catch((error) => {
      res
        .status(400)
        .json({ error: "Problème lors de l'enregistrement du post" });
    });
};
    ...
    .then((post) => {
      ...
      Post.updateOne({ _id: id }, { ...newPost })
        ...
        .catch((error) =>
          res
            .status(401)
            .json({ error: "Problème lors de la modification du post" })
        );
    })
    .catch((error) => {
        ...
        fs.unlink(`images/${filename}`, () => {
          ...
          Post.deleteOne({ _id: req.params.id })
            ...
            .catch((error) =>
              res
                .status(401)
                .json({ error: "Problème lors de la suppression du post" })
            );
        });
      }
    })
